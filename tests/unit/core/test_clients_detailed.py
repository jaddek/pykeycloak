...
class TestKeycloakHttpClientWrapperAsyncDetailed:
    ...
    async def test_request_with_request_error(self):
        ...
        wrapper._client.request = AsyncMock(side_effect=RequestError(message="Network error", request=None))
    ...
    async def test_request_with_http_status_error(self):
        ...
        wrapper._client.request = AsyncMock(side_effect=HTTPStatusError(message="Bad Request", request=None, response=MagicMock()))
...
def test_get_keycloak_client_wrapper_from_env(mock_sanitizer, mock_transport, mock_client):
